...
2. WHILE la descarga de los pesos del modelo está en curso, THE Sistema SHALL mostrar al usuario el progreso de descarga como un porcentaje calculado mediante (bytes descargados ÷ tamaño total en bytes) × 100, redondeado al entero más cercano, actualizado con cada fragmento recibido.
   - Implementación: cubierto en la pantalla de arranque real (`App_State`, `ModelLoadProgressIndicator`) leyendo el `initProgressCallback` que WebLLM ya reporta durante `MLCEngine.reload()` (fase de descarga, carga de pesos en GPU y compilación de shaders), traducido por `modelLoadProgress.ts` -- no mediante el pipeline propio de `Gestor_Descarga_Modelo`, que no se invoca en el arranque real (ver Requisito 2, nota de diseño en `design.md`).
